...
window.LiveEditor = Backbone.View.extend({
    ...
    bind: function bind() {
        ...
        $el.delegate(this.dom.STOP_BUTTON, "click", this.stopCode.bind(this));
...
function addProcessingIframes($el) {
  $($el).find('code.language-prerender, code.language-render').each(function (i, code) {
    ...
    var iframeHtml = '<!DOCTYPE html>\n' + '<link rel="stylesheet" href="css/live-editor.output.css">' + '<body style="height: 100%; margin: 0; overflow: hidden;">' + '<div id="live-editor-output"></div>' + '<script src="js/live-editor.core_deps.js"></script>' + '<script src="js/live-editor.shared.js"></script>' + '<script src="js/live-editor.output_pjs_deps.js"></script>' + '<script src="js/live-editor.output.js"></script>' + '<script src="js/live-editor.output_pjs.js"></script>' + '<script>' + 'var code = ' + JSON.stringify(source) + ';' + 'window.liveEditorOutput = new LiveEditorOutput({' + '  el: $("#live-editor-output")[0],' + '  outputType: "pjs",' + '  noLint: true,' + '  code: code,' + '  width: 400,' + '  height: 400,' + '  autoFocus: true,' + '  workersDir: "workers/",' + '  externalsDir: "external/",' + '  imagesDir: "images/",' + '  soundsDir: "../../sounds/",' + '  execFile: "output.html",' + '  jshintFile: "external/jshint/jshint.js",' + '  newErrorExperience: true,' + '});' + 'window.liveEditorOutput.firstLint = true;' + 'window.liveEditorOutput.runCode(code);' + '</script>' + '</body>';
    ...
    methods: {
      ...
      fullscreen: function fullscreen() {
        ...
        if ('' in params) {
          ...
          var win = window.open('play.html' + _fragment, '_blank');
          win.focus();
        } else {
          (function () {
            // Save with a new id, even if the code is unchanged.
